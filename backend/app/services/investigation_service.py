...
async def _execute_rmm_commands(
    ...
    # Tactical RMM's /runscript/ requires a stored script ID — inline code must use /cmd/.
    # Run each diagnostic command individually via cmd/ and collect labelled output.
    ...
    for cmd in commands:
        try:
            if shell == "powershell":
                inline = f'try {{ {cmd} | Out-String }} catch {{ Write-Host "ERRO: $_" }}'
            else:
                inline = f'{cmd} 2>&1'
