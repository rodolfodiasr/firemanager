...
class SonicWallConnector(BaseConnector):
    ...
    SonicOS 6.x: HTTP Digest Auth on each request (no session needed).
    SonicOS 7.x: same approach works; session-based is also supported.
    ...
    def _client(self) -> httpx.AsyncClient:
        """Return an AsyncClient with Digest Auth — works for all SonicOS versions."""
    ...
    async def test_connection(self) -> ConnectionResult:
        ...
        try:
            async with self._client() as client:
                # Use a real authenticated endpoint to validate credentials
                resp = await client.get("/api/sonicos/access-rules/ipv4")
                if resp.status_code == 401:
                    return ConnectionResult(success=False, error="Authentication failed (401)")
