...
async def list_agents(
    ...
    rows = await db.execute(
        select(EdgeAgent).where(EdgeAgent.tenant_id == current_user.tenant_id)
    )
...
async def register_agent(
    ...
    agent, raw_token = await create_agent(
        db, current_user.tenant_id, body.name, body.notes, current_user.id
    )
...
async def update_agent(
    ...
    agent = await db.scalar(
        select(EdgeAgent).where(
            EdgeAgent.id == agent_id, EdgeAgent.tenant_id == current_user.tenant_id
        )
    )
    if not agent:
...
async def list_installed(
    ...
    rows = await db.execute(
        select(TenantPlugin)
        .where(TenantPlugin.tenant_id == current_user.tenant_id)
...
async def install_plugin_endpoint(
    ...
    install = await install_plugin(db, current_user.tenant_id, plugin_id, current_user.id)
...
async def uninstall_plugin(
    ...
    install = await db.scalar(
        select(TenantPlugin).where(
            TenantPlugin.tenant_id == current_user.tenant_id,
...
async def list_rbac_roles(
    ...
    rows = await db.execute(
        select(RbacCustomRole).where(RbacCustomRole.tenant_id == current_user.tenant_id)
    )
    return rows.scalars().all()
...
async def create_rbac_role(
    ...
    role = RbacCustomRole(tenant_id=current_user.tenant_id, **body.model_dump())
    db.add(role)
    await db.flush()
...
async def delete_rbac_role(
    ...
    role = await db.scalar(
        select(RbacCustomRole).where(
            RbacCustomRole.id == role_id, RbacCustomRole.tenant_id == current_user.tenant_id
        )
    )
    if not role:
...
async def list_assignments(
    ...
    rows = await db.execute(
        select(RbacRoleAssignment).where(
            RbacRoleAssignment.tenant_id == current_user.tenant_id
        )
    )
    return rows.scalars().all()
...
async def assign_rbac_role(
    ...
    role = await db.scalar(
        select(RbacCustomRole).where(
            RbacCustomRole.id == body.role_id,
            RbacCustomRole.tenant_id == current_user.tenant_id,
        )
    )
    if not role:
        raise HTTPException(404, "Role not found")
