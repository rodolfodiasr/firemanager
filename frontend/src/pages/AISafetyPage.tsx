...
import {
  ...
  AlertTriangle, Calendar, Eye, Lock, Siren, ChevronDown, ChevronUp,
} from "lucide-react";
import { aiSafetyApi, type ApprovalRequest, type ErasureRequest, type MaintenanceWindow, type SecurityIncident } from "../api/aiSafety";

type Tab = "windows" | "approvals" | "erasure" | "incidents";
...
function IncidentsTab() {
  ...
  const [timelineInput, setTimelineInput] = useState<Record<string, string>>({});
  const [form, setForm] = useState({
    title: "",
    description: "",
    severity: "medium",
  ...
  const [updateForm, setUpdateForm] = useState<Record<string, { status?: string; root_cause?: string; remediation?: string }>>({});
  ...
  const { data: incidents = [] } = useQuery({
    queryKey: ["aisafety-incidents", statusFilter, severityFilter],
    queryFn: () => aiSafetyApi.listIncidents(params),
  });

  const createMut = useMutation({
    mutationFn: (data: typeof form) =>
      aiSafetyApi.createIncident({
        title: data.title,
        description: data.description || undefined,
        severity: data.severity,
        category: data.category,
        affected_systems: data.affected_systems ? data.affected_systems.split(",").map(s => s.trim()).filter(Boolean) : undefined,
      }),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["aisafety-incidents"] }); setShowCreate(false); setForm({ title: "", description: "", severity: "medium", category: "other", affected_systems: "" }); },
  });

  const updateMut = useMutation({
    mutationFn: ({ id, data }: { id: string; data: Record<string, string | undefined> }) =>
      aiSafetyApi.updateIncident(id, data),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["aisafety-incidents"] }); setUpdateForm({}); },
  });

  const timelineMut = useMutation({
    mutationFn: ({ id, action, details }: { id: string; action: string; details: string }) =>
      aiSafetyApi.addTimeline(id, action, details),
    onSuccess: (_, vars) => {
      qc.invalidateQueries({ queryKey: ["aisafety-incidents"] });
      setTimelineInput(prev => ({ ...prev, [vars.id]: "" }));
    },
  });

  return (
    <div>
      <div className="bg-red-950/30 border border-red-800/50 rounded-xl p-4 mb-4 flex items-start gap-3">
        <Siren size={18} className="text-red-400 mt-0.5 shrink-0" />
        <div className="text-sm text-red-200">
          <strong>SIRP — Registro de Incidentes de Segurança</strong>. Registre, investigue e acompanhe incidentes com timeline auditada.
        </div>
      </div>
      ...
      <div className="flex items-center justify-between mb-4 gap-3 flex-wrap">
        <div className="flex gap-2 flex-wrap">
          <select value={statusFilter} onChange={e => setStatusFilter(e.target.value)}
            ...
            {Object.entries(INCIDENT_STATUS_LABELS).map(([v, l]) => <option key={v} value={v}>{l}</option>)}
          </select>
      ...
      {showCreate && (
        <div className="bg-gray-800 border border-gray-700 rounded-xl p-5 mb-4">
          <h3 className="text-white font-semibold mb-4">Registrar Incidente de Segurança</h3>
          <div className="space-y-3 mb-4">
            <div>
              <label className="text-xs text-gray-400 block mb-1">Título *</label>
              <input value={form.title} onChange={e => setForm({...form, title: e.target.value})}
                placeholder="Ex: Tentativa de acesso não autorizado ao firewall de SP"
                className="w-full bg-gray-700 border border-gray-600 rounded-lg px-3 py-2 text-white text-sm" />
            </div>
            <div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="text-xs text-gray-400 block mb-1">Severidade</label>
            ...
            <div>
              <label className="text-xs text-gray-400 block mb-1">Sistemas afetados (separados por vírgula)</label>
              <input value={form.affected_systems} onChange={e => setForm({...form, affected_systems: e.target.value})}
                placeholder="fw-sp-01, sw-core-01"
                className="w-full bg-gray-700 border border-gray-600 rounded-lg px-3 py-2 text-white text-sm" />
            </div>
          </div>
          <div className="flex justify-end gap-2">
      ...
      <div className="space-y-3">
        {incidents.length === 0 && (
          <div className="text-center text-gray-500 py-12">Nenhum incidente registrado.</div>
        )}
        {incidents.map(inc => {
          const isExpanded = expandedId === inc.id;
          const uf = updateForm[inc.id] ?? {};
          return (
            <div key={inc.id} className="bg-gray-800 border border-gray-700 rounded-xl overflow-hidden">
              {/* Header */}
              <div className="p-4 cursor-pointer" onClick={() => setExpandedId(isExpanded ? null : inc.id)}>
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-1 flex-wrap">
                      <span className={`text-xs px-2 py-0.5 rounded font-semibold ${SEVERITY_BADGE[inc.severity] ?? ""}`}>
                        {inc.severity.toUpperCase()}
                      </span>
                      <span className={`text-xs px-2 py-0.5 rounded ${INCIDENT_STATUS_BADGE[inc.status] ?? ""}`}>
                    ...
                    <div className="text-white font-medium text-sm">{inc.title}</div>
                    {inc.description && <p className="text-gray-400 text-xs mt-1">{inc.description}</p>}
                    <div className="text-xs text-gray-500 mt-1">
                      {fmtDt(inc.created_at)}
                      {inc.affected_systems && inc.affected_systems.length > 0 &&
                        ` · Sistemas: ${inc.affected_systems.join(", ")}`}
                    </div>
                  </div>
              ...
              {isExpanded && (
                <div className="border-t border-gray-700 p-4 space-y-4">
                  {/* Status update */}
                  <div>
                    <label className="text-xs text-gray-400 block mb-2">Atualizar status</label>
                    <div className="flex gap-2 flex-wrap">
                      {["open", "investigating", "contained", "resolved", "closed"].map(s => (
                        <button key={s} onClick={() => updateMut.mutate({ id: inc.id, data: { status: s } })}
                  ...
                  <div className="grid grid-cols-1 gap-3">
                    <div>
                      <label className="text-xs text-gray-400 block mb-1">Causa raiz</label>
                      <textarea
                        value={uf.root_cause ?? inc.root_cause ?? ""}
                        onChange={e => setUpdateForm(prev => ({ ...prev, [inc.id]: { ...prev[inc.id], root_cause: e.target.value } }))}
                        rows={2}
                    ...
                    <div>
                      <label className="text-xs text-gray-400 block mb-1">Remediação</label>
                      <textarea
                        value={uf.remediation ?? inc.remediation ?? ""}
                        onChange={e => setUpdateForm(prev => ({ ...prev, [inc.id]: { ...prev[inc.id], remediation: e.target.value } }))}
                        rows={2}
                        placeholder="Ações de remediação tomadas ou planejadas..."
                        className="w-full bg-gray-700 border border-gray-600 rounded-lg px-3 py-2 text-white text-sm"
                      />
                    </div>
                    {(uf.root_cause !== undefined || uf.remediation !== undefined) && (
                      <div className="flex justify-end">
                        <button onClick={() => updateMut.mutate({ id: inc.id, data: { root_cause: uf.root_cause, remediation: uf.remediation } })}
                          disabled={updateMut.isPending}
                          className="bg-brand-600 hover:bg-brand-700 text-white px-3 py-1.5 rounded-lg text-xs">
                          Salvar
                        </button>
                      </div>
                    )}
                  ...
                  {/* Timeline */}
                  <div>
                    <label className="text-xs text-gray-400 block mb-2">Timeline ({inc.timeline.length} eventos)</label>
                    <div className="space-y-2 max-h-48 overflow-y-auto mb-3">
                      {inc.timeline.map((entry, idx) => (
                        <div key={idx} className="flex gap-2 text-xs">
                          <span className="text-gray-500 shrink-0">{fmtDt(entry.at)}</span>
                          <span className="text-gray-300"><strong>{entry.action}</strong>{entry.details ? ` — ${entry.details}` : ""}</span>
                        </div>
                      ))}
                    </div>
                    <div className="flex gap-2">
                      <input
                        value={timelineInput[inc.id] ?? ""}
                        onChange={e => setTimelineInput(prev => ({ ...prev, [inc.id]: e.target.value }))}
                        placeholder="Adicionar entrada na timeline..."
                        className="flex-1 bg-gray-700 border border-gray-600 rounded-lg px-3 py-2 text-white text-xs"
                        onKeyDown={e => {
                          if (e.key === "Enter" && timelineInput[inc.id]) {
                            timelineMut.mutate({ id: inc.id, action: timelineInput[inc.id], details: "" });
                          }
                        }}
                      />
                      <button
                        onClick={() => { if (timelineInput[inc.id]) timelineMut.mutate({ id: inc.id, action: timelineInput[inc.id], details: "" }); }}
                        disabled={!timelineInput[inc.id] || timelineMut.isPending}
...
export function AISafetyPage() {
  ...
  const TABS: { id: Tab; label: string; icon: JSX.Element }[] = [
    ...
    { id: "incidents", label: "SIRP (Incidentes)", icon: <Siren size={16} /> },
    ...
    <main className="flex-1 overflow-auto bg-gray-950 p-6">
      <div className="max-w-5xl mx-auto">
        ...
        <p className="text-gray-400 text-sm mb-6">
          Janelas de manutenção, aprovação dupla (four-eyes), registro de incidentes (SIRP) e direito à eliminação (LGPD Art. 18).
        </p>
