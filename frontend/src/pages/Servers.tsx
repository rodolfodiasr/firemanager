...
function ServerModal({ initial, onClose }: ModalProps) {
  ...
  const { register, handleSubmit, formState: { errors, isSubmitting } } = useForm<ServerCreate>({
    defaultValues: initial
      ...
      : { ssh_port: 22, os_type: "linux", credentials: { username: "" } },
  });
      ...
      <div className="bg-white rounded-xl w-full max-w-lg shadow-xl">
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200">
          <h2 className="font-semibold text-gray-900">
            {initial ? "Editar Servidor" : "Registrar Servidor SSH"}
          </h2>
        ...
        <form onSubmit={handleSubmit((d) => mut.mutate(d))} className="p-6 space-y-4">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="sm:col-span-1">
              <label className="block text-xs font-medium text-gray-700 mb-1">Porta SSH</label>
              <input
                type="number"
            ...
            <div className="col-span-2">
              ...
              <input
                ...
                placeholder="Servidor web de produção"
                className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500"
              />
            </div>
          </div>
          ...
          <div className="border-t border-gray-100 pt-4">
            <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-3">
              Credenciais SSH {initial && "(deixe vazio para manter atual)"}
            </p>
            <div className="space-y-3">
              ...
              <div>
                <label className="block text-xs font-medium text-gray-700 mb-1">
                  Chave Privada (PEM) — alternativa à senha
                </label>
                <textarea
...
function ServerRow({ server, onEdit, onDelete }: RowProps) {
  const qc = useQueryClient();
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <p className="text-sm text-gray-500 mt-1">
            Registre servidores Linux/Windows via SSH para análise pelo Analista N3.
          </p>
        </div>
        <button
          onClick={openCreate}
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col items-center justify-center py-16 text-center">
          ...
          <p className="text-sm text-gray-400 mt-1 max-w-sm">
            Registre servidores Linux via SSH para coletar diagnósticos e analisar com IA.
          </p>
          <button
            onClick={openCreate}
        ...
        <div className="space-y-3">
          {servers.map((s) => (
            <ServerRow key={s.id} server={s} onEdit={() => openEdit(s)} onDelete={() => {}} />
          ))}
        </div>
      )}
