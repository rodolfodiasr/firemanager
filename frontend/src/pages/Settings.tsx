                            ...
                            <tr key={intent.key} className="border-t border-gray-100 hover:bg-gray-50">
                              ...
                              <td className="px-4 py-2.5 text-center">
                                {hasOverride ? (
                                  <button
                                    onClick={() =>
                                      deleteMutation.mutate({ scopeType: "user", scopeId: selectedUserId, intent: intent.key })
                                    }
...
export function Settings() {
  ...
  const isAdmin = user?.is_super_admin || tenantRole === "admin";
  const [tab, setTab] = useState<"conta" | "integracoes" | "politica">("conta");

  return (
    <PageWrapper title="Configurações">
      {/* Tabs */}
      <div className="flex gap-1 mb-6 border-b border-gray-200">
        {(["conta", "integracoes", ...(isAdmin ? ["politica"] : [])] as const).map((t) => (
          <button
            key={t}
            onClick={() => setTab(t as "conta" | "integracoes" | "politica")}
          ...
          >
            {t === "conta" ? "Conta" : t === "integracoes" ? "Integrações" : "Política"}
          </button>
        ))}
      </div>
